...
// Cập nhật người dùng đang chỉnh sửa
        ...
        if (editingUser) {
          ...
          quanLyPhong
            ...
            .then((response) => {
              ...
              const updatedRooms = rooms.map((room) => room.id === editingUser.id ? response.data.content : room);
              setRooms(updatedRooms);
              openNotificationWithIcon('success', 'Thành công', 'Phòng đã được cập nhật');
            })
            .finally(() => {
              setIsModalVisible(false);
              setEditingUser(null);
          ...
          quanLyPhong
            ...
            .then((response) => {
              ...
              openNotificationWithIcon('success', 'Thành công', 'Phòng đã được thêm');
            })
            .finally(() => {
              setIsModalVisible(false);
              setIsLoading(false);
    ...
    const columns = [
        ...
        //     'bg-gray-300', },
        { title: 'Tên', dataIndex: 'tenPhong', key: 'tenPhong', className:
            'bg-gray-300', },
        { title: 'Khách', dataIndex: 'khach', key: 'khach', className:
            'bg-gray-300', },
        { title: 'Phòng ngủ', dataIndex: 'phongNgu', key: 'phongNgu', className:
            'bg-gray-300', },
        { title: 'Giường', dataIndex: 'giuong', key: 'giuong', className:
            'bg-gray-300', },
        { title: 'Phòng tắm', dataIndex: 'phongTam', key: 'phongTam', className:
            'bg-gray-300', },
        { title: 'Mô tả', dataIndex: 'moTa', key: 'moTa', className:
            'bg-gray-300 ' },
        { title: 'Giá tiền', dataIndex: 'giaTien', key: 'giaTien', className:
            'bg-gray-300', },
        { title: 'Mã vị trí', dataIndex: 'maViTri', key: 'maViTri', className:
            'bg-gray-300', },
        {
            title: 'Hình ảnh',
            dataIndex: 'hinhAnh',
        ...
        <div>
          ...
          <Input.Search placeholder="Search..." enterButton ={<i className="fa-solid fa-magnifying-glass"></i>} className="ml-3 py-1 px-2 leading-tight focus:outline-none" />
            ...
            <div className="overflow-x-auto mt-10">
                <Table columns={columns} dataSource={rooms} rowKey="id" loading={isLoading} imageUrl={imageUrl}  />
            </div>
        </div>
    );
};
